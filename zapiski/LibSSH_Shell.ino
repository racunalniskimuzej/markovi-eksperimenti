// ESP32 libssh port.
//
// Iodeo, created on 8/10/2021
// Based on Ewan Parker, April 2020.
//
...
int ex_main(){
    ...
    fprintf(stderr, "User name (e.g. pi) ? \n");
    ...
    fcntl(0, F_SETFL, fcntl(0, F_GETFL) | O_NONBLOCK); // set stdin (0) to nonblocking mode
         ...
         !ssh_channel_is_eof(channel))
    {
      ...
      // write to client terminal
      wbytes = 0;
      while (wbytes < rbytes) {
        c = buffer[wbytes];        
        if (escFlag) {
          escFlag = false;
          if (c == '[') ignore = false; // keep CSI code only
          else ignore = true; // be sure to detect end of esc code --> see below
        }
        if (c == 0x1b) escFlag = true;
        if (!ignore) {
          if (write(1, buffer+wbytes, 1) < 0) 
            goto failed;
        }
        else { // search end of Ansi esc code
          if (c == 0x7) { //BEL for titles
            ignore = false;
          }
        }
        wbytes++;
      ...
      rbytes = read(0, buffer, sizeof(buffer)); // 0 refers to stdin
      if (rbytes < 0) {
        usleep(50000L); // 0.05 second
      }
      //write to remote server
      if (rbytes > 0) {
        wbytes = ssh_channel_write(channel, buffer, rbytes);
        if (wbytes != rbytes) 
          goto failed;
      }
    }
