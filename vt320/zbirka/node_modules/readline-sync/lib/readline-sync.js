...
function _readlineSync(options) {
    ...
    while (true) {
      ...
      if (chunk) {
        if (chunk == "\x7f" && input.length > 0) {
          if (options.mask != '') { fs.writeSync(fdW, "\b\x1b[0K"); }
          input = input.slice(0, -1);
        }
        if (/\x1b\[[A-D]$/.test(rawInput) && options.mask != '') {
          chunk = '';
          fs.writeSync(fdW, (new Array(input.length + 1)).join("\b") + "\x1b[0K" +
          (options.mask.includes("return") ? new Function("str", options.mask)(lastInput) : lastInput));
          input = lastInput;
        }
      ...
      if (chunk) {
        if (!isCooked) {
          if (!options.hideEchoBack) {
            ...
            if (options.mask.includes("return")) {
              fs.writeSync(fdW, new Function("str", options.mask)(chunk));
            } else {
  ...
  return options.displayOnly ? '' :
    (lastInput = options.keepWhitespace || options.keyIn ? input : (input != '' ? input.trim()
     : (options.hideEchoBack && options.mask == '' ? lastInput.trim() : input.trim())));
}
