...
function _readlineSync(options) {
  ...
  (function() { // try read
    ...
    while (true) {
      ...
      if (chunk) {
        if (chunk == "\x7f" && input.length > 0 && options.mask != '') {
          fs.writeSync(fdW, "\b\x1b[0K");
          input = input.slice(0, -1);
        }
        if (chunk == "\x1b[A" && options.mask != '') {
          fs.writeSync(fdW, (new Array(input.length + 1)).join("\b") + "\x1b[0K" + lastInput);
          input = lastInput;
        }
        chunk = chunk.replace(/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]|\[[A-D]/g, '');
      }
